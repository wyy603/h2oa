...
from legged_gym.utils.diff_quat import flip_quat_by_w, quat_to_vec6d, quat_multiply, quat_inv, broadcast_quat_apply, broadcast_quat_multiply
...
class H1Walk(BaseTask):
    ...
    def check_termination(self):
        ...
        self.extras["termination"]['termination_contact_buf'] = self.reset_buf
        self.extras["termination"]['r_threshold_buff'] = self.reset_buf
        self.extras["termination"]['p_threshold_buff'] = self.reset_buf
        self.extras["termination"]['z_threshold_buff'] = self.reset_buf
        self.extras["termination"]['has_fallen'] = self.reset_buf
    ...
    def _reward_hip_yaw_dof_error(self):
        ...
        return torch.sum(torch.square(hip_diff), dim=1)
...
# from legged_gym.envs.h1.h1_config import H1HistoryCfg


# class H1WalkWithHistory(H1Walk):
#     cfg: H1HistoryCfg

#     def _init_buffers(self):
#         super()._init_buffers()
#         self.history_interval = 10
#         self.history_buffer = HistoryBuffer(self.num_envs, self.cfg.env.num_history, self.cfg.env.num_prop)

#     def reset_idx(self, env_ids):
#         if len(env_ids) == 0:
#             return
#         super().reset_idx(env_ids)
#         self.history_buffer.reset_idx(env_ids)
...
#         if self.add_noise:
#             obs_buf += (2 * torch.rand_like(self.obs_buf) - 1) * self.noise_scale_vec

#         update_ids = torch.nonzero(self.episode_length_buf % self.history_interval == 0).squeeze(1)
#         history_buffer = self.history_buffer.get_history_buffer()
#         flatten_history_buffer = torch.flatten(history_buffer - obs_buf.unsqueeze(1), 1, 2)
#         self.obs_buf = torch.cat([obs_buf, flatten_history_buffer], dim=-1)

#         # sparse history
#         self.history_buffer.update_idx(update_ids, obs_buf[update_ids])