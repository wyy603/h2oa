...
from legged_gym.envs.h1.h1_mcsr_config import H1RoughCfg
...
class H1Walk(BaseTask):
    ...
    def update_target_pkls(self, reset_env_ids):
        ...
        self.target_jt_future = self.target_jt_seq[frame_id_flatten]#.to(self.device)
        self.target_jt_pos_future = self.target_jt_future[:,:19]
    ...
    def compute_observations(self):
        ...
        # breakpoint()
        target_jt_pos_future = self.target_jt_pos_future.reshape(self.num_envs, -1, self.num_dofs) - self.default_dof_pos
        target_jt_pos_future = target_jt_pos_future.reshape(self.num_envs, -1)
        self.obs_buf = torch.cat((
            # self.commands,  # 3
            target_jt_pos_future * self.obs_scales.dof_pos,
            ...
            self.target_yaw_vel,  # 3 TODO lin xy ang yaw; align with reward
            projected_gravity,  # 3
    ...
    def _reward_target_updof(self):
        # Penalize distance to target joint angles
        target_jt_pos = self.target_jt_pos#+self.default_dof_pos # NOTE(xh): add default pos to target jt
        target_jt_pos_error = torch.mean(torch.abs(self.dof_pos - target_jt_pos)[:,self.upper_dof_indices], dim=1)
        return torch.exp(-4 * target_jt_pos_error), target_jt_pos_error
