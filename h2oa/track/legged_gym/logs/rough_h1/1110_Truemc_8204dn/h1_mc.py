...
from .h1_mc_config import H1RoughCfg
...
class H1():
    ...
    def _super_init(self, cfg, sim_params, physics_engine, sim_device, headless):
        ...
        if self.cfg.env.obs.target_root_ang_vel:
            num_task += 3
        self.num_obs += num_task*self.cfg.env.obs.future
        self.num_privileged_obs = self.num_obs + cfg.env.num_privileged_obs if cfg.env.num_privileged_obs is not None else None
    ...
    def _init_target_pkls(self):
        ...
        self.target_jt_j_future = torch.zeros(self.num_envs, self.cfg.env.obs.future, dtype=torch.long, device=self.device)
    ...
    def update_target_pkls(self, reset_env_ids):
        ...
        # self.target_jt_j_future: shape self.num_envs, self.cfg.env.obs.future, from self.target_jt_j+1 to self.target_jt_j+future
        j_future = torch.arange(0, self.cfg.env.obs.future, device=self.device, dtype=torch.long) * self.cfg.env.obs.future_stride
        self.target_jt_j_future = self.target_jt_j[:,None] + j_future[None,:]
        # self.target_jt_j_future not bigger than self.target_jt_seq_len
        self.target_jt_j_future = torch.minimum(self.target_seq_len[self.target_jt_i,None] - 1, self.target_jt_j_future)



        t1 = time.time()
        # self.target_jt = torch.stack([self.target_jt_seq[i][j] for i, j in zip(self.target_jt_i, self.target_jt_j)], dim=0).to(self.device)
        self.target_jt = self.target_jt_seq[self.motion_start_id[self.target_jt_i]+self.target_jt_j]#.to(self.device)
        self.target_jt_pos, self.target_jt_vel = self.target_jt[:,:19], self.target_jt[:,19:]

        # self.target_global = torch.stack([self.target_global_seq[i][j] for i, j in zip(self.target_jt_i, self.target_jt_j)], dim=0).to(self.device)
        self.target_global = self.target_global_seq[self.motion_start_id[self.target_jt_i]+self.target_jt_j]#.to(self.device)
        ...
        frame_id_flatten = (self.motion_start_id[self.target_jt_i][:,None]+self.target_jt_j_future).reshape(-1)
        self.target_global_future = self.target_global_seq[frame_id_flatten]#.to(self.device)
        self.target_global_pos_future, self.target_global_ori_future, self.target_global_vel_future, self.target_global_ang_vel_future = \
            self.target_global_future[:,:3*self.num_bodies], \
            self.target_global_future[:,3*self.num_bodies:9*self.num_bodies], \
            self.target_global_future[:,9*self.num_bodies:12*self.num_bodies], \
            self.target_global_future[:,12*self.num_bodies:15*self.num_bodies]
        self.target_heading_quat_inv_future = diff_rot.calc_heading_quat_inv(vec6d_to_quat(self.target_global_ori_future[:,:6].reshape(-1,3,2)))[:,None]
    ...
    def update_proprio_latency_buf(self): # TODO check if change real-time
        ...
        self.dof_pos_latency.update(self.dof_pos)
        self.dof_vel_latency.update(self.dof_vel)
        self.body_pos_latency.update(self.body_pos)
        self.body_ori_latency.update(self.body_ori)
        self.base_ang_vel_latency.update(self.base_quat)
        self.projected_gravity_latency.update(self.projected_gravity)
    ...
    def compute_observations(self):
        ...
        ##################### task #####################
        self.heading_quat_inv_future = self.heading_quat_inv.repeat(1, self.cfg.env.obs.future,1).reshape(self.num_envs * self.cfg.env.obs.future, 1, 4)
        self.heading_quat_future = self.heading_quat.repeat(1, self.cfg.env.obs.future,1).reshape(self.num_envs * self.cfg.env.obs.future, 1, 4)

        if self.cfg.env.obs.target_global_pos: # 3 * (num_dofs + 1) wtf heading frame
            # target_global_pos = self.target_global_pos.reshape(-1, self.num_bodies, 3) - (self.root_pos-self.env_origins)[:,None]
            # target_global_pos = broadcast_quat_apply(self.heading_quat_inv, target_global_pos).reshape(-1, 3 * (self.num_bodies))
            
            target_global_pos_future = self.target_global_pos_future.reshape(self.num_envs, self.cfg.env.obs.future, self.num_bodies, 3) - (self.root_pos-self.env_origins)[:,None,None]
            target_global_pos_future = target_global_pos_future.reshape(self.num_envs * self.cfg.env.obs.future, self.num_bodies, 3)
            target_global_pos_future = broadcast_quat_apply(self.heading_quat_inv_future, target_global_pos_future).reshape(self.num_envs, self.cfg.env.obs.future * 3 * (self.num_bodies))
            
            # obs_list.append(target_global_pos * self.obs_scales.target_global_pos)
            obs_list.append(target_global_pos_future * self.obs_scales.target_global_pos)
        if self.cfg.env.obs.target_global_ori: # 6 * (num_dofs + 1) wtf heading frame
            ...
            #     vec6d_to_quat(self.target_global_ori.reshape(B,self.num_bodies,3,2)))
            # target_global_ori = quat_to_vec6d(target_global_ori).reshape(-1, 6 * (self.num_bodies))

            target_global_ori_future = broadcast_quat_multiply(self.heading_quat_inv_future,
                vec6d_to_quat(self.target_global_ori_future.reshape(B*self.cfg.env.obs.future,self.num_bodies,3,2)))
            target_global_ori_future = quat_to_vec6d(target_global_ori_future).reshape(self.num_envs, self.cfg.env.obs.future * 6 * (self.num_bodies))

            # obs_list.append(target_global_ori * self.obs_scales.target_global_ori)
            obs_list.append(target_global_ori_future * self.obs_scales.target_global_ori)

        if self.cfg.env.obs.diff_global_pos: # 3 * (num_dofs + 1)
            # diff_global_pos = self.target_global_pos.reshape(B, self.num_bodies, 3) - (self.body_pos_raw - self.env_origins[:,None])
            # diff_global_pos = broadcast_quat_apply(self.heading_quat_inv, diff_global_pos).reshape(-1, 3 * (self.num_bodies))

            diff_global_pos_future = self.target_global_pos_future.reshape(B, -1, self.num_bodies, 3) - ((self.body_pos_raw - self.env_origins[:,None]))[:,None]
            diff_global_pos_future = diff_global_pos_future.reshape(self.num_envs* self.cfg.env.obs.future, self.num_bodies, 3)
            diff_global_pos_future = broadcast_quat_apply(self.heading_quat_inv_future, diff_global_pos_future).reshape(self.num_envs, self.cfg.env.obs.future * 3 * (self.num_bodies))

            # obs_list.append(diff_global_pos * self.obs_scales.diff_global_pos)
            obs_list.append(diff_global_pos_future * self.obs_scales.diff_global_pos)
        if self.cfg.env.obs.diff_global_ori: # 6 * (num_dofs + 1) 
            # diff_global_body_rot = broadcast_quat_multiply(vec6d_to_quat(self.target_global_ori.reshape(B,self.num_bodies,3,2)), 
            ...
            diff_global_body_rot_future = broadcast_quat_multiply(vec6d_to_quat(self.target_global_ori_future.reshape(B,-1,self.num_bodies,3,2)), 
                                    diff_rot.quat_conjugate(self.body_ori_raw)[:,None]).reshape(self.num_envs* self.cfg.env.obs.future, self.num_bodies, 4)
            diff_global_body_rot_future = broadcast_quat_multiply(self.heading_quat_inv_future,
                                    broadcast_quat_multiply(diff_global_body_rot_future, 
                                    self.heading_quat_future))
            diff_global_body_rot_future = quat_to_vec6d(diff_global_body_rot_future).reshape(self.num_envs, self.cfg.env.obs.future * 6 * (self.num_bodies))
            
            # obs_list.append(diff_global_body_rot * self.obs_scales.diff_global_ori)
            obs_list.append(diff_global_body_rot_future * self.obs_scales.diff_global_ori)
        ...
        if self.cfg.env.obs.target_root_vel: # 3 local
            ...
            target_root_local_vel_future = broadcast_quat_apply(self.target_heading_quat_inv_future.squeeze(1), 
                                                self.target_global_vel_future[:,:3]).reshape(self.num_envs, self.cfg.env.obs.future,  3)
            diff_root_local_vel_future = target_root_local_vel_future - self.base_lin_vel[:,None]
            diff_root_local_vel_future = diff_root_local_vel_future.reshape(self.num_envs, self.cfg.env.obs.future * 3)

            # obs_list.append(diff_root_local_vel * self.obs_scales.target_root_vel)
            obs_list.append(diff_root_local_vel_future * self.obs_scales.target_root_vel)
            # obs_list.append(self.target_body_vel[:,:3] * self.obs_scales.target_root_vel)
        if self.cfg.env.obs.target_root_ang_vel: # 3 local
            ...
            target_root_local_ang_vel_future = broadcast_quat_apply(self.target_heading_quat_inv_future.squeeze(1), 
                                                self.target_global_ang_vel_future[:,:3]).reshape(self.num_envs, self.cfg.env.obs.future,  3)
            diff_root_local_ang_vel_future = target_root_local_ang_vel_future - self.base_ang_vel[:,None]
            diff_root_local_ang_vel_future = diff_root_local_ang_vel_future.reshape(self.num_envs, self.cfg.env.obs.future * 3)

            # obs_list.append(diff_root_local_ang_vel * self.obs_scales.target_root_ang_vel) 
            obs_list.append(diff_root_local_ang_vel_future * self.obs_scales.target_root_ang_vel)
            # obs_list.append(self.target_body_ang_vel[:,:3] * self.obs_scales.target_root_ang_vel) 
    ...
    def _reward_gr_MaskedMimic(self):
        # breakpoint()
        global_ori = quat_to_vec6d(flip_quat_by_w(self.body_ori_raw), do_normalize=True).reshape(-1,self.num_bodies,6)
        target_gr_error = (torch.sum(torch.square(self.target_global_ori.reshape(-1,self.num_bodies,6) - global_ori), dim=1)).mean(dim=1)
    ...
    def _reward_jv_MaskedMimic(self):
        # breakpoint()
        target_jv_error = (torch.sum(torch.square(self.target_global_vel.reshape(-1,self.num_bodies,3) - self.body_vel_raw), dim=1)).mean(dim=1)
        return torch.exp(-0.08 * target_jv_error), target_jv_error
    
    def _reward_jav_MaskedMimic(self):
        # breakpoint()
        target_jav_error = (torch.sum(torch.square(self.target_global_ang_vel.reshape(-1,self.num_bodies,3) - self.body_ang_vel_raw), dim=1)).mean(dim=1)
    ...
    def _reward_global_pos_phc(self):
        ...
        discrete_pose_reward0 =  0.3*(diff_global_body_pos < 0.0015).to(diff_global_body_pos)
