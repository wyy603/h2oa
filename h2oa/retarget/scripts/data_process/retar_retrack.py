...
recycle_file='/home/ubuntu/data/PHC/recycle_4277.pkl'
...
for data_pair in recycle_data_list:
    if data_pair is None:
        continue
    name = data_pair['name']
    # if '0:0-SOMA_soma_subject1_sit_001_stageii' not in name and '0:0-ACCAD_Female1Walking_c3d_B21_s2_-_put_down_box_to_walk_stageii' not in name: # 0:0-ACCAD_Female1Walking_c3d_B21_s2_-_put_down_box_to_walk_stageii
    #     continue
    rotlist = []
    # print(amass_name)
    ...
    target_jt_A = torch.from_numpy(data_pair['jt_A'])#.to(device)
    ...
    target_jt_B = torch.from_numpy(data_pair['jt_B'])#.to(device)
    ...
    for i in (range(target_jt_A.shape[0])):
        ...
        root_state_tensor[0, :3] = target_global_pos_A[i]
        root_state_tensor[1, :3] = target_global_pos_B[i]
        # root_state_tensor[1, 2] = root_state_tensor[1, 0] + 2
        root_state_tensor[0, 3:7] = vec6d_to_quat(target_global_ori_A[i].reshape(3,2))
        ...
        gym.refresh_dof_state_tensor(sim)
        gym.refresh_rigid_body_state_tensor(sim)
        error_global = abs((data_pair['global_B'][i,:3*20] - data_pair['global_A'][i,:3*20])).mean().item()
        # if target_global_pos_B[:,2].min() < lowest_pelvis:
        #     lowest_name = name
        #     lowest_data = data_pair
        #     lowest_pelvis = target_global_pos_B[:,2].min()
        ...
        # break
    error_global_1 += abs((data_pair['global_A'] - data_pair['global_B'])[:,:3*20]).mean().item()
...
# joblib.dump(lowest_data, "/home/ubuntu/data/PHC/lowest_data.pkl")
