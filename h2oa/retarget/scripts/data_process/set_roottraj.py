...
test_file = '/home/ubuntu/data/PHC/nvel_dn_35_mdm.pkl'
...
# traj_file='/cephfs_yili/shared/xuehan/H1_RL/amass_17268.pkl' 
traj_file = '/home/ubuntu/data/PHC/tk15_walk_mdm.pkl'
...
traj_data_list = joblib.load(traj_file) 
...
for test_id, (test_name, test_data) in tqdm(enumerate(test_data_dict.items())):
    ...
    if  show and gym.query_viewer_has_closed(viewer):
        break
    if test_name not in traj_data_list:# or test_name not in names:
        continue
    # breakpoint()
    # print(traj_data_list.keys())
    ...
    #     continue
    traj_data = traj_data_list[test_name]
    ...
    dof_h1_vel[1: , :] = (dof_h1[1: , :] - dof_h1[:-1, :]) * FPS
    ...
    if 'globalv2' in test_data:
        assert test_data['globalv2'].shape[-1] == h1_rigid_body_num * 7
        root_pos_rs = torch.from_numpy(traj_data['globalv2'][:test_length])[:,:3] 
        root_ori_rs = torch.from_numpy(traj_data['globalv2'][:test_length])[:,h1_rigid_body_num*3:h1_rigid_body_num*3+4] # NOTE(xh) visualize for check??
    elif 'global' in test_data:
        assert test_data['global'].shape[-1] == h1_rigid_body_num * 15
        root_pos_rs = torch.from_numpy(traj_data['global'][:test_length])[:,:3] 
        root_ori_rs = torch.from_numpy(traj_data['global'][:test_length])[:,h1_rigid_body_num*3:h1_rigid_body_num*3+6] # NOTE(xh) visualize for check??
    else:
        root_pos_rs = torch.from_numpy(traj_data['root'][:test_length])[:,:3] #.to(device)
        root_ori_rs = torch.from_numpy(traj_data['root'][:test_length])[:,3:] #.to(device)

    # root_pos_h1[...,-1] = root_pos_h1[...,-1] + .12
    root_pos_h1[...,:2] = root_pos_rs[...,:2]
    root_ori_rs = vec6d_to_quat(root_ori_rs.reshape(-1,3,2)) if root_ori_rs.shape[-1] == 6 else root_ori_rs
    root_ori_h1 = vec6d_to_quat(root_ori_h1.reshape(-1,3,2)) if root_ori_h1.shape[-1] == 6 else root_ori_h1
    # tmp=broadcast_quat_multiply(
    #         calc_heading_quat_inv(root_ori_h1),
    #         calc_heading_quat(root_ori_h1))
    # # print(tmp)
    # breakpoint()
    root_ori_h1 = broadcast_quat_multiply(
        root_ori_h1,
        broadcast_quat_multiply(
            calc_heading_quat_inv(root_ori_h1),
            calc_heading_quat(root_ori_rs)))
    # root_ori_h1 = broadcast_quat_multiply(
    #     broadcast_quat_multiply(
    #         calc_heading_quat(root_ori_rs),
    #         calc_heading_quat_inv(root_ori_h1)),
    ...
    for i in (range(dof_h1.shape[0])):
        ...
        root_pos=root_pos_h1[i] 
        # root_pos[...,-1] = root_pos[...,-1] - rigid_body_tensor[:h1_rigid_body_num, 2].min()
        root_state_tensor[0, :3] = root_pos
        ...
        root_ori_quat = vec6d_to_quat(root_ori_h1[i].reshape(3,2)) if root_ori_h1.shape[-1] == 6 else root_ori_h1[i]
        root_ori_quat = torch.from_numpy((R(root_ori_quat)*off1).as_quat())
        root_state_tensor[0, 3:7] = root_ori_quat
        ...
        #     # 刷新图表显示
        #     plt.pause(0.001)
    # delta_1motion_max = torch.tensor(delta_1motion_max[5:]).max()
    ...
    if save:
        ...
        global_vel[1:] = (global_pos[1:] - global_pos[:-1])*FPS
        ...
        global_ang_vel[1:] = quat_to_rotvec(diff_quat).reshape(-1,h1_rigid_body_num,3)*FPS
...
out_path = os.path.join(os.path.dirname(test_file), f"setroot_{len(motion_pkls)}.pkl")
with open(log_path, "a") as f:
    # write in csv file

    f.write(f"{out_path}, {succ_rate.to(torch.float64).mean():.4g}, {endeffector_error_mean:.4g}, {jt_error_mean:.4g}, /, /, /, {test_length_sum/len(test_data_dict):.4g}\n")
if save:
    print(out_path)
