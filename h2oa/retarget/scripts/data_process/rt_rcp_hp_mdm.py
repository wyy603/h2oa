...
def load_motion_with_skeleton_remap_exp(curr_file, skeleton_tree, retarget_json):
...
in_file='/home/ubuntu/data/MDM/mdm_35hand.pkl' 
# in_file1='/home/xuehan/MotionLCM/mdm_10.pkl' 
# in_file2='/home/xuehan/MotionLCM/results.pkl' 
# in_file1='/home/ubuntu/data/PHC/mdm_10.pkl' 
# in_file='/home/ubuntu/data/PHC/data/amass/pkls/amass_isaac_train_0.pkl' 
amass_data_list = joblib.load(in_file) 
# amass_data_list1 = joblib.load(in_file1) 
# amass_data_list2 = joblib.load(in_file2) 
# amass_data_list1 = joblib.load(in_file1) 
amass_data_list = {k:v for k,v in amass_data_list.items() if 'hand' in k}
...
for motoin_id, (amass_name, amass_data) in tqdm(enumerate(amass_data_list.items()),total=len(amass_data_list)):
    ...
    if  show and gym.query_viewer_has_closed(viewer):
        break
    source_motion, retarget_motion = load_motion_with_skeleton_remap_exp(amass_data, skeleton_tree, retarget_json)
    # if tmp==0:
    ...
    # local_rotation = source_motion.local_rotation.contiguous().clone()[:,1:] # motion.local_rotation[:,0] should be identical to root_transformation[:,4], different from the root_transformation_xh
    root_transformation_xh = source_motion.root_transformation_xh.contiguous().clone()
    # motion_len = local_rotation.shape[0]
    ...
    # root_rotation_6d = quat_to_vec6d(root_rotation).reshape(-1,6)
    # local_rotation_6d = quat_to_vec6d(local_rotation).reshape(motion_len,23,6)

    # if save:
    #     human_motion_pkls[amass_name] = {
    #         "root_transformation": torch.concat([root_translation, root_rotation_6d], dim=1), # [n, 6+3]
    #         "local_rotation": local_rotation_6d, # [n, 23, 6]
    ...
    dof_h1[:,[13,17]] = dof_14[:,[22,31]].to(dof_h1) #,23,21,22,25,32,30,31,34
    ...
    for i in (range(root_transformation_xh.shape[0])):
        # sleep(0.01)
        if fixed:
            i=13
        if show_plot:
        ...
        # root_state_tensor[:1, 3:7] = torch.from_numpy((R(root_rotation[i])*R(initial_root_rot)*sRot.from_euler('y', root_pitch[i])).as_quat())
        ...
        root_state_tensor[:1, 3:7] = torch.from_numpy((R(initial_root_rot)*R(root_rotation[i])*off1).as_quat())
        # root_state_tensor[2, 3:7] = torch.from_numpy((R(initial_root_rot)*R(root_rotation[i])*off2).as_quat())

        # root_state_tensor[1:2, 3:7] = torch.from_numpy((R(initial_root_rot)*R(root_rotation[i])).as_quat())
...
# filename = f'rcphp_{len(motion_pkls)}_swingwalk_mdm.pkl' 
filename = in_file.replace('mdm', 'rcphp') 
joblib.dump(motion_pkls, filename)
# filename = f'human_{len(motion_pkls)}_swingwalk_mdm.pkl' 
# joblib.dump(human_motion_pkls, f"/cephfs_yili/shared/xuehan/H1_RL/{filename}")
