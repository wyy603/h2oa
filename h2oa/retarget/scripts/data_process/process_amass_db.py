...
def process_qpos_list(qpos_list):
    ...
    for (k, v) in tqdm(pbar):
        ...
        gender = v["gender"]
        try:
            amass_fr = v["mocap_frame_rate"] if "mocap_frame_rate" in v.keys() else v["mocap_framerate"]
        except:
            # from IPython import embed;embed()
            # amass_fr=target_fr
            removed_k.append(k)
...
amass_splits = {
    'vald': ['HumanEva', 'HDM05', 'SFU', 'mosh'], # 28 215 44 77
    'test': ['Transitions', 'SSM'], # 110 30
    'train': ['CMU', 'PosePrior', 'HUMAN4D', 'TotalCapture', "SOMA", 'CNRS', 'Eyes_Japan_Dataset', 'KIT', 'BML', 'EKUT', 'TCD', "DanceDB", "ACCAD", "BioMotionLab", "Eyes", "DFaust"],  # Adding ACCAD
    # 'train': ['CMU', 'MPI_Limits'=PosePrior, 'TotalCapture', 'Eyes_Japan_Dataset', 'KIT', 'BML', 'EKUT', 'TCD_handMocap', "BMLhandball", "DanceDB", "ACCAD", "BMLmovi", "BioMotionLab", "Eyes", "DFaust"]  # Adding ACCAD
}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", type=str, default="/cephfs_yili/backup/xuehan/dataset/PHC/amass_db_smplh.pt")
    ...
    joblib.dump(train_data, f"/cephfs_yili/shared/xuehan/H1_RL/amass_train_{len(train_data)}.pkl")
    joblib.dump(test_data, f"/cephfs_yili/shared/xuehan/H1_RL/amass_test_{len(test_data)}.pkl")
    joblib.dump(valid_data, f"/cephfs_yili/shared/xuehan/H1_RL/amass_valid_{len(valid_data)}.pkl")
