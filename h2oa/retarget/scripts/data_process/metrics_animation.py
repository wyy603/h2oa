...
test_file='/home/ubuntu/data/AMASS/dn_25.pkl'
# test_file='/home/ubuntu/data/AMASS/tk8089_1105mc8204dn_dn_8198_h2o.pkl'
# test_file='/home/ubuntu/data/AMASS/tkh2o_25.pkl'
...
color=(255/255,97/255,3/255) if 'dn' in test_file else (60/255, 106/255, 106/255)
...
# while(1):
    ...
    for i in (range(global_translation.shape[0])):
        ...
        root_state_tensor[0, :3] = torch.tensor(R(initial_root_rot).apply(root_translation[i].numpy()))
        root_state_tensor[0, 2] = root_pos_h1[i,2]
        ...
        if 'h2o' in test_file:
            root_state_tensor[0, 3:7] = torch.from_numpy((R(initial_root_rot)*R(root_rot_h1_)).as_quat())
        else:
            root_state_tensor[0, 3:7] = root_rot_h1_
        ...
        cam_pos = gymapi.Vec3(*((root_state_tensor[1, :3] + np.array([-1,-2,1])).tolist()))
