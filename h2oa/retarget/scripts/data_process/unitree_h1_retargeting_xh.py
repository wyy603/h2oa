...
for motoin_id, (amass_name, amass_data) in enumerate(amass_data_list.items()):
    ...
    for i in tqdm(range(global_translation.shape[0])):
        ...
        gym.set_dof_state_tensor_indexed(sim,
                                    ...
                                    gymtorch.unwrap_tensor(env_id), len(env_id))
        ...
        root_state_tensor[:1, 3:7] = torch.from_numpy((sRot.from_quat(root_rotatoin[i])*sRot.from_euler('y', root_pitch[i])).as_quat())
