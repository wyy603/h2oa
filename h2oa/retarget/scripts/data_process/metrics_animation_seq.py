...
# create viewer
if show:
    viewer_options = gymapi.CameraProperties()
    # viewer_options.clear_color = (0.1, 0.1, 0.1)  
    viewer = gym.create_viewer(sim, viewer_options)
    if viewer is None:
...
# test_file='/home/ubuntu/data/MDM/rcphp_10swingwalk.pkl'
# test_file='/home/ubuntu/data/MDM/rcp_10swingwalk.pkl'
# test_file='/home/ubuntu/data/MDM/pop_10_swingwalk.pkl'
test_file='/home/ubuntu/data/MDM/pophm_10_swingwalk.pkl'
# test_file='/home/ubuntu/data/MDM/dn_10_swingwalk_mdm.pkl'
...
color=(135/255, 206/255, 255/255) if 'dn' in test_file else (60/255, 179/255, 113/255)
for i in range(actor_num):
    ...
    for t in range(22):
        gym.set_rigid_body_color(env,actor_handle,t,gymapi.MESH_VISUAL_AND_COLLISION, gymapi.Vec3(
            *color
            # 255/255,97/255,3/255 # orange
            # 202/255,235/255,216/255
            # 60/255, 179/255, 113/255 # green
            # blue
            # 255/255, 218/255, 185/255,
            # 255/255, 106/255, 106/255,
            ))
...
for i in range(actor_num):
    ...
    # for t in range(24):
    #     gym.set_rigid_body_color(env,actor_handle_ref,t,gymapi.MESH_VISUAL_AND_COLLISION, gymapi.Vec3(0,0.0,0))
...
if show:
    ...
    # cam_pos = gymapi.Vec3(0, -5, 1)
    cam_pos = gymapi.Vec3(5, 1, 1)
...
#     continue
print(test_name)
amass_data = list(amass_data_list.values())[33]
...
root_pos_h1[:,-1] = root_pos_h1[:,-1] + 0.15 -5
for i in range(actor_num):
    frame_id = 70
    # frame_id = 17 #45
    ...
    # if i ==0:
    # if i > 0:
    #     root_state_tensor[i, 1] = root_state_tensor[i, 1] + torch.rand_like(root_state_tensor[i, 1])*0.2
    # if i > 0:
        # root_state_tensor[i, 1] = root_state_tensor[i, 1] + 0.5*i
    root_state_tensor[actor_num+i, :3] = torch.tensor(R(initial_root_rot).apply(root_translation[frame_id].numpy()))
    # if i == 0:
    #     root_state_tensor[actor_num+i, 1] = root_state_tensor[actor_num+i, 1] - 0.15
    # if i > 0:
        # root_state_tensor[actor_num+i, 1] = root_state_tensor[actor_num+i, 1] + 0.5*i
    # if i > 0:
    #     root_state_tensor[actor_num+i, 2] = root_state_tensor[actor_num+i, 2] + 0.02*i
