...
in_file='/home/ubuntu/data/MDM/mdm_10swingwalk.pkl' 
...
for motoin_id, (amass_name, amass_data) in tqdm(enumerate(amass_data_list.items()),total=len(amass_data_list)):
    ...
    local_rotation = motion.local_rotation.contiguous().clone()[:,1:] # motion.local_rotation[:,0] should be identical to root_transformation[:,4], different from the root_transformation_xh
    root_transformation_xh = motion.root_transformation_xh.contiguous().clone()
    ...
    root_translation[:,2] = root_translation[:,2] + 0.13
...
# filename = f'rcp_{len(motion_pkls)}_swingwalk_mdm.pkl' 
filename = in_file.replace('mdm', 'rcp') 
joblib.dump(motion_pkls, filename)
# filename = f'human_{len(motion_pkls)}_swingwalk_mdm.pkl' 
filename = in_file.replace('mdm', 'human') 
joblib.dump(human_motion_pkls, filename)
