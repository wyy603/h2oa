...
if show:
    ...
    # # 设置光源参数
    # light_index = 0  # 光源索引
    # light_intensity = gymapi.Vec3(1.0, 1.0, 1.0)  # 光源的强度（RGB）
    # ambient_intensity = gymapi.Vec3(1, 1, 1) # 环境光的强度（RGB）
    # light_direction = gymapi.Vec3(0.0, -1.0, -1.0)  # 光源的方向（单位向量）
    #
    # # 设置光源
    # gym.set_light_parameters(sim, light_index, light_intensity, ambient_intensity, light_direction)
...
squat_joint_angles = {
    'left_hip_yaw_joint': 0.0, # 0
    'left_hip_roll_joint': 0.0,
    'left_hip_pitch_joint': -2.57,
    'left_knee_joint': 2,
    'left_ankle_joint': -0.585,
    'right_hip_yaw_joint': 0.0, # 5
    'right_hip_roll_joint': 0.0,
    'right_hip_pitch_joint': -2.57,
    'right_knee_joint': 2,
    ...
    'left_elbow_joint': 1.57,
    'right_shoulder_pitch_joint': -1.2, # 15
...
root_state_tensor[1,2]-=0.49
root_state_tensor[1, 3:7] = torch.from_numpy( R.from_euler('y', 1.15).as_quat())
...
hip_names = [s for s in h1_rigid_body_names if ('hip' in s and 'roll' in s)]
shoulder_pos=[]
...
hip_pos=[]
hip_pos.append(rigid_body_states[0, h1_rigid_body_names.index(hip_names[0]), :3])
hip_pos.append(rigid_body_states[0, h1_rigid_body_names.index(hip_names[1]), :3])
hip_pos.append(rigid_body_states[1, h1_rigid_body_names.index(hip_names[0]), :3])
hip_pos.append(rigid_body_states[1, h1_rigid_body_names.index(hip_names[1]), :3])
print(shoulder_pos, hip_pos)
