...
def local_rotation_to_dof_h1(local_rot,conpen,zero=False):
    ...
    L_Shoulder_conpen,L_Elbow_rot2 = rot_decompose(L_Elbow_rot,np.array([1,0,0],dtype=np.float64))

    L_Elbow_angle, _ = rot_decompose(L_Elbow_rot2, np.array([0,0,1],dtype=np.float64))
    ...
    R_Shoulder_conpen, R_Elbow_rot2 = rot_decompose(R_Elbow_rot,np.array([1,0,0],dtype=np.float64))

    R_Elbow_angle, _ = rot_decompose(R_Elbow_rot2, np.array([0,0,-1],dtype=np.float64))
...
mrm_file='/home/ubuntu/data/PHC/mrm_0.pkl' 
# mrm_file='/home/ubuntu/data/PHC/retarget_fail2464_amass_train_13912.pkl' 
# mrm_file='/cephfs_yili/shared/xuehan/H1_RL/amass_train_13912.pkl' 
mrm_data_list = joblib.load(mrm_file) # dict_keys(['pose_aa', 'pose_6d', 'trans', 'beta', 'seq_name', 'gender'])
mrm_motion = torch.from_numpy(mrm_data_list['motion']).permute(2, 0, 1).contiguous()
...
root_translation = mrm_motion[:, -1, :].clone()
...
root_rotatoin = axis_angle_to_quaternion(mrm_motion[:, 0, :].clone())

# root_translation = root_translation[:,[0,2,1]] # x, z, y
# root_translation[:,2] *= -1

dof_smpl = mrm_motion[:, 1:-1, :].clone().reshape(-1,69) # [259, 69]

conpen1 = True
conpen2 = False
fixed = False
dof_h1, root_pitch = local_rotation_to_dof_h1(axis_angle_to_quaternion(mrm_motion[:, 1:-1, :].clone()), conpen=conpen1,zero=False) # [259, 19]
...
dof_h1_vel[1: , :] = (dof_h1[1: , :] - dof_h1[:-1, :]) * 20
...
root_state_tensor[1:2, :3] = torch.tensor(R(initial_root_rot).apply(root_translation[i].numpy()))
