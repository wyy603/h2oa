...
# axisy = R(rotation).as_matrix()[:,1]
# Rp = np.cross(axisy,[0,1,0])
# Rp = Rp / np.linalg.norm(Rp)
# Rp *= np.arccos(axisy.dot([0,1,0])) # axis angle
# Ry = (R.from_rotvec(Rp) * R(rotation)).as_quat()
# Rxz = (R(Ry).inv()*R(rotation)).as_quat()
...
def rot_decompose(rot, axis):
    ...
    Rp = axis3 / torch.norm(axis3,dim=1)[:,None] * theta[:,None]
...
in_file='/home/ubuntu/data/PHC/results.pkl' 
...
for motoin_id, (amass_name, amass_data) in tqdm(enumerate(amass_data_list.items())):
    ...
    # motion = load_motion_with_skeleton_exp(torch.from_numpy(amass_data).permute(2,0,1), skeleton_tree)
    # if tmp==0:
    #     motion = load_motion_with_skeleton(amass_data, skeleton_tree)
    #     tmp=1
    #     continue
    ...
    # global_translation[:] = global_translation[:,:,[1,0,2]] # x, z, y
    # global_translation[:,:,0] = -global_translation[:,:,0]

    # global_translation[:,:,0] = global_translation[:,:,0] - root_translation[0,0] # TODO
    # global_translation[:,:,1] = global_translation[:,:,1] - root_translation[0,1] # TODO
    # global_translation[:,:,2] = global_translation[:,:,2] + 0.14
    ...
    # dof_smpl = to_torch(amass_data['opt_pose'][:,3:]).cpu()
    ...
    for i in (range(global_translation.shape[0])):
        ...
        ################
        # ret = chain.forward_kinematics(h1_dof_states['pos'])
        # left_hand_link = ret['left_hand_link']
        # left_hand_tg = left_hand_link.get_matrix()[:,:3,3]
        # left_hand_global = quat_apply(base_ori, left_hand_tg) + base_pos
        #############
        # print(root_state_tensor[:1].shape)
        # print(root_state_tensor.shape)
        root_state_list.append(root_state_tensor[:1,:7].cpu().clone().numpy())
        global_body_pos = rigid_body_tensor_copy[:20, :3] # 60
        global_body_ori = rigid_body_tensor_copy[:20, 3:7]
        ...
        global_body_info_list.append(rigid_body_tensor_copy[:20, :7])
        ...
        delta = global_translation[i,3] - rigid_body_tensor_copy[5, :3]

        if show_plot:
            rotlist.append(delta.norm().item())
            ax.cla()
            # 重新绘制数据
    ...
    global_ang_vel[1:] = quat_to_rotvec(diff_quat).reshape(-1,20,3)*motion.fps
    global_ang_vel[0] = global_ang_vel[1]
    global_motion = np.concatenate([global_pos.numpy().reshape(-1,20*3), 
        quat_to_vec6d(global_ori).reshape(-1,20,6).numpy().reshape(-1,20*6), 
        global_vel.reshape(-1,20*3), 
        global_ang_vel.reshape(-1,20*3)], axis=-1) # NOTE reshape before concat!!!!!!!
