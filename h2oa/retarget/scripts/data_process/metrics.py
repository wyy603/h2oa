...
threshold1 = 0.5
import glob
test_file='/cephfs_yili/shared/xuehan/H1_RL/tk8089_1105mc8204dn_dn_8198_h2o.pkl'
...
test_data_dict_ori={}
for file in glob.glob(test_file):
    test_data_dict_ori.update(joblib.load(file)) # dict_keys(['pose_aa', 'pose_6d', 'trans', 'beta', 'seq_name', 'gender'])

if 'MDM' not in test_file:
    names_h2o = list(joblib.load('/cephfs_yili/shared/xuehan/H1_RL/tk7902_1106mc8204rcp_rcp_8204_h2o.pkl').keys()) # NOTE
    test_data_dict = {}
    for name, data in test_data_dict_ori.items():
        if name in names_h2o:# and name not in names_32:
            test_data_dict[name] = data
    del test_data_dict_ori
else:
...
save = save if 'jtroot_' in test_file or 'rename' in test_file else False
assert 'jtroot_' in test_file or 'rename' in test_file if save else True
...
for test_id, (test_name, test_data) in tqdm(enumerate(test_data_dict.items())):
    ...
    if 'jt' in test_data:
        jt_test = test_data['jt']
    elif 'dof' in test_data:
        jt_test = test_data['dof']

    test_length =  jt_test.shape[0]
    ...
    dof_h1 = torch.from_numpy(jt_test[:test_length])[...,:19]#.to(device)
    ...
    if 'globalv2' in test_data:
        ...
        test_rigid_body_num = test_data['global'].shape[-1] // 15
        assert test_rigid_body_num == 22 or test_rigid_body_num == 24
        root_pos_h1 = torch.from_numpy(test_data['global'][:test_length])[:,:3] 
        root_ori_h1 = torch.from_numpy(test_data['global'][:test_length])[:,test_rigid_body_num*3:test_rigid_body_num*3+6] # NOTE(xh) visualize for check??
    elif 'root_trans_offset' in test_data:
        # assert test_data['global'].shape[-1] == h1_rigid_body_num * 15
        root_pos_h1 = torch.from_numpy(test_data['root_trans_offset'][:test_length])
        root_ori_h1 = torch.from_numpy(test_data['root_rot'][:test_length])
    else:
    ...
    jt_errormax_all.append(jt_error.abs().max())
...
succ_rate = succ_rate0 * succ_rate1
...
out_path = test_file.replace("_rename", "_reform") if "_rename" in test_file else test_file
