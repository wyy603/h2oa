...
def local_rotation_to_dof_h1(local_rot):
    ...
    Torso_euler=torch.atan2(Torso_v[...,1], Torso_v[...,0])[...,None] # z
    root_pitch=torch.atan2(Torso_v[...,0], Torso_v[...,2])-math.pi/2 # y
    ...
    L_Elbow_v = torch.from_numpy(sRot.from_quat(L_Elbow_rot).apply([0,1,0]))

    L_Elbow_v2 = torch.from_numpy(sRot.from_quat(L_Elbow_rot).apply([0,0,1]))

    L_Elbow_yaw = torch.atan2(L_Elbow_v[...,2], L_Elbow_v[...,0]) # z  z/x
    
    L_Elbow_rotvec = torch.from_numpy(sRot.from_quat(L_Elbow_rot).as_rotvec())
    # print(L_Elbow_rotvec[0])
    # print(L_Elbow_rotvec.shape)
    # print(L_Elbow_rotvec[0])
    # L_Elbow_angle = L_Elbow_rotvec[:,1:2] # torch.norm(L_Elbow_rotvec,dim=1,keepdim=True)
    # print(L_Elbow_angle.shape)
    # L_Elbow_euler[:,1:2]
    ...
    L_Shoulder_yaw = torch.zeros_like(L_Shoulder_yaw)
    
    L_Shoulder_euler=torch.stack([L_Shoulder_pitch,L_Shoulder_roll,L_Shoulder_euler[:,0]],dim=-1)
    

    L_Elbow_euler=-torch.arccos(L_Elbow_v[...,1:2])+math.pi/2 # y

    L_Elbow_angle = L_Elbow_euler
    ...
    R_Shoulder_euler = torch.from_numpy(sRot.from_quat(R_Shoulder_rot).as_euler('zxy'))
    
    R_Shoulder_yaw = torch.zeros_like(R_Shoulder_yaw)
    R_Shoulder_euler=torch.stack([R_Shoulder_pitch, R_Shoulder_roll, R_Shoulder_euler[:,0]], dim=-1)
    R_Elbow_euler=-torch.arccos(-R_Elbow_v[...,1:2])+math.pi/2

    dof_pos = torch.cat([L_Hip_euler, L_Knee_euler[:,:1], L_Ankle_euler[:,:1], R_Hip_euler, R_Knee_euler[:,:1], R_Ankle_euler[:,:1], Torso_euler, L_Shoulder_euler, L_Elbow_angle, R_Shoulder_euler, R_Elbow_euler[:,:1]], dim=1)
    # print('!!!')
    # print(dof_pos.shape) # torch.Size([282, 19])
    # print(root_pitch.shape) # torch.Size([282])
...
for motoin_id, (amass_name, amass_data) in enumerate(amass_data_list.items()):
    ...
    root_translation[:,2] = root_translation[:,2] + 0.08
    ...
    dof_h1, root_pitch = local_rotation_to_dof_h1(motion.local_rotation) # [259, 19]
    ...
    for i in (range(global_translation.shape[0])):
        ...
        root_state_tensor[:2, :3] = torch.tensor(R(initial_root_rot).apply(root_translation[i].numpy()))
        # root_state_tensor[:1, 1] -= 1.5
        ...
        root_state_tensor[:1, 3:7] = torch.from_numpy((R(initial_root_rot)*R(root_rotatoin[i])*sRot.from_euler('y', root_pitch[i])).as_quat())
    ...
    amass_name = f'{amass_name}_{motoin_id}'
    if motoin_id <= 2:
        np.save("../HST/isaacgym/h1_motion_data/" + "jt_" + amass_name + ".npy", dof_motion) #[n, 19x2]
        np.save("../HST/isaacgym/h1_motion_data/" + "body_" + amass_name + ".npy", local_motion) #[n, (1+19)x(3+6+3+3)]
