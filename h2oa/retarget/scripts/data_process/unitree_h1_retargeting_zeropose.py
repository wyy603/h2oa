...
in_file='data/amass/pkls/amass_isaac_test_0.pkl' 
...
for motoin_id, (amass_name, amass_data) in enumerate(amass_data_list.items()):
    ...
    for i in tqdm(range(global_translation.shape[0])):
        ...
        h1_dof_states['pos'] = dof_h1[i]*0 + offset
        h1_dof_states['vel'] = dof_h1_vel[i]*0 #TODO
        ...
        # root_state_tensor_zeros[:1, :3] = root_translation[i]
        # # root_state_tensor_zeros[:1, 3:7] = torch.from_numpy((sRot.from_quat(global_rotation[i,0]) ).as_quat()) #  sRot.from_quat(global_rotation[i,0]) * sRot.from_quat([-0.707107, 0.0, 0.0, 0.707107]) 
        # root_state_tensor_zeros[:1, 3:7] = torch.from_numpy((sRot.from_quat(root_rotatoin[i])*sRot.from_euler('y', root_pitch[i])).as_quat())
        # root_state_tensor_zeros[1] = root_state_tensor[1]
        # gym.set_actor_root_state_tensor(sim, gymtorch.unwrap_tensor(root_state_tensor_zeros))
    ...
    breakpoint()
    if motoin_id == 1:
        amass_name = 'default_pose'
        np.save("../HST/isaacgym/h1_motion_data/" + "jt_" + amass_name + ".npy", dof_motion*0) #[n, 19x2]
