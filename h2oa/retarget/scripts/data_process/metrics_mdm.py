...
threshold0 = 1
threshold1 = 1
test_file='/home/ubuntu/data/MDM/rcp_35_mdm_1028_lctk_walkforward_hipyaw_newmotion_dr_resumepolicy_60000.pkl'
...
amass_file='/home/ubuntu/data/MDM/mdm_35.pkl' 
...
for test_id, (test_name, test_data) in tqdm(enumerate(test_data_dict.items())):
    ...
    torque_mean =  abs(test_data['torque']).mean()
    torque_list.append(torque_mean)
    ...
    global_pos_vel = _compute_velocity(root_pos_h1[:,None], time_delta)
    global_pos_vel[...,0:1] = 0.6 - global_pos_vel[...,0:1]
    # breakpoint()
    global_ori_ang_vel = _compute_angular_velocity(root_ori_h1[:,None], time_delta)
    lin_vel_list.append(global_pos_vel.norm(dim=-1).mean())
    ang_vel_list.append(global_ori_ang_vel.norm(dim=-1).mean())
    ...
    for i in (range(global_translation.shape[0])):
        ...
        gt_pos_global = global_translation[i,human_metric_id]
        gt_root = global_translation[i,0]
        gt_heading_inv = calc_heading_quat_inv(root_state_tensor[1:2, 3:7])
        gt_pos_heading = broadcast_quat_apply(gt_heading_inv, gt_pos_global - gt_root)

        test_pos_global = rigid_body_tensor[h1_metric_id, :3]
        test_root = rigid_body_tensor[0, :3]
        test_heading_inv = calc_heading_quat_inv(root_state_tensor[0:1, 3:7])
        test_pos_heading = broadcast_quat_apply(test_heading_inv, test_pos_global - test_root)

        delta = (gt_pos_heading - test_pos_heading).norm(dim=-1).mean()
        delta_max = (gt_pos_heading - test_pos_heading).norm(dim=-1).max()
...
# succ_rate0 = torch.tensor(endeffector_error_all) < threshold0
# succ_rate1 = torch.tensor(jt_error_all) < threshold1
# succ_rate = torch.logical_and(succ_rate0, succ_rate1)
succ_rate0 = torch.clamp_min((threshold0 - torch.tensor(endeffector_error_all))/threshold0, 0)
succ_rate1 = torch.clamp_min((threshold1 - torch.tensor(jt_error_all))/threshold1, 0)
succ_rate2 = torch.clamp_min((threshold0 - torch.tensor(lin_vel_list))/threshold0, 0)
succ_rate3 = torch.clamp_min((threshold1 - torch.tensor(ang_vel_list))/threshold1, 0)
succ_rate = succ_rate0 * succ_rate1* succ_rate2* succ_rate3
endeffector_error_mean=torch.tensor(endeffector_error_all).mean()
jt_error_mean=torch.tensor(jt_error_all).mean()
lin_vel_mean=torch.tensor(lin_vel_list).mean()
ang_vel_mean=torch.tensor(ang_vel_list).mean()
torque_mean = torch.tensor(torque_list).mean()
# breakpoint()
...
with open(log_path, "a") as f:
    ...
    f.write(f"{out_path}, {succ_rate.to(torch.float64).mean():.4g}, {endeffector_error_mean:.4g}, {jt_error_mean:.4g}, {lin_vel_mean:.4g}, {ang_vel_mean:.4g}, {torque_mean:.4g}, {test_length_sum/len(test_data_dict):.4g}\n")
if save:
    print(out_path)
