...
# test_file='/cephfs_yili/shared/xuehan/H1_RL/retarget_64_amass_train_13912.pkl'
# test_file='/home/ubuntu/data/PHC/retarget_64_amass_train_13912.pkl'
test_file='/cephfs_yili/shared/xuehan/H1_RL/tk_8544_30__filt_retar_noz_retarget_13911_amass_train_13912.pkl'
...
motion_pkls={}
jt_error_all=[[] for _ in range(6)]
test_length_sum=[0]*6
endeffector_error_all=[[] for _ in range(6)]
endeffector_errormax_all=[[] for _ in range(6)]
for test_id, (test_name, test_data) in tqdm(enumerate(test_data_dict.items())):
    # if 'walk' not in amass_name:
    #     continue
    split_id = names_map[test_name]
    print(test_name, split_id)
    ...
    test_length_sum[split_id] += test_length
    ...
    jt_error_all[split_id].append(jt_error_mean)
    ...
    endeffector_error_all[split_id].append(delta_1motion_mean)
    endeffector_errormax_all[split_id].append(delta_1motion_max)
    if save:
...
with open(log_path, "a") as f:
    ...
    for i in range(6):
        ...
        succ_rate = (torch.tensor(endeffector_errormax_all[i]) < threshold).to(torch.float64)
        endeffector_error_mean=torch.tensor(endeffector_error_all[i]).mean()
        jt_error_mean=torch.tensor(jt_error_all[i]).mean()
        f.write(f"{succ_rate.sum()}-{succ_rate.shape[0]}, {succ_rate.mean():.4g}, {endeffector_error_mean:.4g}, {jt_error_mean:.4g}, /, /, /, /, {test_length_sum[i]/(succ_rate.shape[0]):.4g}\n")
if save:
    print(out_path)
