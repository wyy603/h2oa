...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', default='data/amass/pkls/amass_isaac_im_train_take6_upright_slim.pkl')
    ...
    # copy_keys = [k.replace(f"{network_name_prefix}.{trained_idx}", f"{network_name_prefix}.{trained_idx + 1}") for k in loading_keys]
    # loading_keys = [k for k in checkpoint['model'].keys() if k.startswith(f"{network_name_prefix}")] 
    ...
    for idx, key_name in enumerate(copy_keys):
        ...
        # except:
        #     checkpoint['model'][key_name] = (checkpoint['model'][loading_keys[idx]]).clone()
